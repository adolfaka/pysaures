...
object_add = api.object_add(city='Moscow', street='Sadovaya', building='75', utc=3)
object_id = object_add.data.id #11753

controller_add = api.sensor_add_first_step(sn='CC50E3FFFFFF')
...
meters_add = api.sensor_add_second_step(sn='CC50E3FFFFFF', object_id=object_id, devices=meters)
print(meters_add.status) #ok

firmware_update = api.sensor_settings(sn='CC50E3FFFFFF', name='Home controller', new_firmware='4.4.1')
print(firmware_update.status) #ok

date_now = datetime.now().replace(microsecond=0).isoformat() #YYYY-MM-DDThh:mm:ss
meters_data = api.object_meters(id=11901, date=date_now)
